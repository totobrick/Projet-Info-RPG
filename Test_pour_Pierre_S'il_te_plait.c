...
void Event(card *c, Player *p, int e1, int e2, int e3, int e4, int e5, int e6,
           int e7, int e8, int e9, int e10, int e11);

void perso_move(Player *p, card *tab, int size,
                int key); // key = KEY_UP ou KEY_DOWN ou KEY_RIGHT ou KEY_LEFT
void play(Player *p, card *tab, int size, WINDOW *win, int e1, int e2, int e3,
          int e4, int e5, int e6, int e7, int e8, int e9, int e10, int e11);
void choose_weapon(Player *p, WINDOW *win);
void interaction_card(Player *p, card *c, int key, WINDOW *win, int e1, int e2,
                      int e3, int e4, int e5, int e6, int e7, int e8, int e9,
                      int e10, int e11, int size);
...
void generate_board(card *tab, int size) {
  char cards[] = {'e', 'e', 'e', 'e', 'e', 'e', 'e', 'b', 'b', 'b', 'b',
                  'z', 'z', 'z', 'z', 't', 't', 't', 't', 'h', 'h', 'h',
                  'h', 'C', 'C', 'T', 'T', 'E', 'B', 'G', 'D', 'P'};
    ...
    for (int j = 1; j < (size - 1); j++) { // Colonnes tableau
      if (remaining_card > 0) {
        ...
        // portail
          ...
          printf(" OUPS ! Le jeu n'a pas pu être créé. La carte générée "
                 "aléatoirement n'est pas dans le paquet.\n");
          exit(1);
        }
        ...
        printf(
            "Toutes les cartes du jeu sont placées !\n"); // est affiché 4 fois
                                                          // car il y a 4 cases
                                                          // vides pour mettre
                                                          // les joueurs
      }
...
void Portal (Player* p, card* tab, int size, WINDOW* win){
    ...
    do{
        ...
        new_direction_y = 0;
        do{
            wprintw(win, "\nChoisissez les coordonnés x, entre 1 et 6, vers
lesquels vous voulez allez!"); wrefresh(win); new_direction_x = getch(); }while
(new_direction_x < '1' || new_direction_x > '6');

        do{
            wprintw(win,"\nChoisissez les coordonnés y, entre 1 et 6, vers
lesquels vous voulez allez!"); wrefresh(win); new_direction_y = getch(); }while
...
int e4, int e5, int e6, int e7, int e8, int e9, int e10, int e11){
        ...
        int forbidden=0;
        card* newcard;				// newcard : pointe vers la
carte ou on veut aller

        //si le joueur est coincé entre 4 murs (ou cartes dévoilé) -> MORT
        if (  ( (*(tab + ((*p).y-1)*size + (*p).x)).wall == 1 || (*(tab +
((*p).y-1)*size + (*p).x)).hidden == 1 )  &&  ( (*(tab + ((*p).y+1)*size +
(*p).x)).wall == 1 || (*(tab + ((*p).y+1)*size + (*p).x)).hidden == 1 )  &&  (
(*(tab + ((*p).y)*size + (*p).x-1)).wall == 1 || (*(tab + ((*p).y)*size +
(*p).x-1)).hidden == 1 )  &&  ( (*(tab + ((*p).y)*size + (*p).x+1)).wall == 1 ||
(*(tab + ((*p).y)*size + (*p).x+1)).hidden == 1)  ){
            (*p).life=0;
            ...
            wrefresh(win);
                ...
                do{
                        forbidden=0;
                        // indique si le joueur a le droit de se déplacer sur la
case où il veut aller
                                //    forbidden = 0       -> c'est ok
                                //    forbidden = 1       -> c'est un mur ou une
carte retournée do{ wprintw(win, "Où voulez-vous vous déplacer (utilisez les
flèches de votre clavier): "); wrefresh(win); forbidden=0; key = getch(); if
(key!=KEY_UP && key!=KEY_DOWN && key!=KEY_RIGHT && key!=KEY_LEFT){ forbidden =
1; wprintw(win, "\nMouvement entré invalide, veuillez réessayer.");
                                }
                        } while(forbidden==1);           //tant qu'on ne
rencontre pas un mur. ATTENTION au cas où le joueur est coincé entre 4 murs !!!
-> le faire mourir.
...
+ (*p).x);
                                ...
                                case KEY_RIGHT:
                                        if ( (*(tab + ((*p).y)*size +
(*p).x+1)).wall == 1 || (*(tab + ((*p).y)*size + (*p).x+1)).hidden == 1){
                                                forbidden = 1;
                                        }
                                        else{
                                                newcard = (tab + ((*p).y)*size +
(*p).x+1);
                                        }
                                        break;
...
(*p).x-1);
        ...
        refresh();
        sleep(3);
        interaction_card(p, newcard, key, win, e1, e2, e3, e4, e5, e6, e7, e8,
e9, e10, e11, size);
        //bouger que si on gagne : void interaction_card(Player* p, card* c);
...
void interaction_card(Player* p, card* c, int key, WINDOW* win, int e1, int e2,
int e3, int e4, int e5, int e6, int e7, int e8, int e9, int e10, int e11, int
size){
        ...
        else if ((*c).r.type[0]==1){ //Vérifie si la classe Ranger est sur la
bonne relique if ((*p).class==3){
                (*p).relic=1;
        ...
        else if ((*c).r.type[1]==1){ //Vérifie si la classe Guerrier est sur la
bonne relique if ((*p).class==2){
                (*p).relic=1;
                printw ("Le joueur vient de trouver sa relique.\n");
            }
            else{
        ...
        else if ((*c).r.type[2]==1){ //Vérifie si la classe Magicien est sur la
bonne relique if((*p).class==1){
                (*p).relic=1;
                printw ("Le joueur vient de trouver sa relique.");
            }
            else{
        ...
        else if ((*c).r.type[3]==1){ //Vérifie si la classe Voleur est sur la
bonne relique if((*p).class==4){
                (*p).relic=1;
                printw ("Le joueur vient de trouver sa relique.");
            }
            else{
...
cf PROCEDURE ALEXIS ATTENTION INITIALISATION EVENT
        }
        else if ((*c).type[2]==1){
            Portal(p,c, size, win);
        }
            ...
            //Totem(p,c); -> Exchang_Totem (Player P, card c,card new_card,card
tempo);
            (*p).life=0;
        }
...
void perso_move(Player* p, card* tab, int size, int key){		//key =
KEY_UP ou KEY_DOWN ou KEY_RIGHT ou KEY_LEFT

            switch (key){           //Vérifie que la saisie de déplacement est
correcte (pas de char) et possible (pas vers un wall) case KEY_UP :
                    (*p).y ++;					//le joueur va sur la
case break;
...
case break;
                ...
                default :
                    printw("ERREUR au moment de la saisie de déplacement du
joueur. Veuillez réessayer avec les flèches haut, bas, droite ou gauche");
                    break;
            }
    refresh();
}
...
int main() {
  ...
  if (menu_select == 1) {
    ...
    create_player(p1, win_menu); // Créée l'identité du joueur 1
    move(0, 0);
    printw("TEST");
    refresh();
    sleep(2);
    ...
    create_player(p2, win_menu); // Créée l'identité du joueur 2
    (*p2).num = 2;
    ...
    if (nmb_player >= 3) {
      create_player(p3, win_menu); // Créée l'identité du joueur 3
      (*p3).num = 3;
    ...
    if (nmb_player == 4) {
      create_player(p4, win_menu); // Créée l'identité du joueur 4
      (*p4).num = 4;
    ...
    game = malloc(SIZE * SIZE *
                  sizeof(card)); // game est notre plateau de jeu (tableau)
    if (game == NULL) {
      printf("Problème d'allocation de mémoire pour la création du tableau du "
             "jeu.\n");
      exit(10);
    } /*
     Player* tab_player[] = {p1, p2, p3, p4};             //tableau où il y a
     les 4 joueurs int r=-1; do{ r++; r = r%nmb_player;
             (*(tab_player[r])).life = 1;
             do{
                     play(tab_player[r], game, SIZE, win_interface, e1, e2, e3,
     e4, e5, e6, e7, e8, e9, e10, e11);			// INTERACTION JOUEUR }
     while( (*(tab_player[r])).life==1 );

     } while ( (*(tab_player[r])).relic!=1 || (*(tab_player[r])).treasure!=1 );
